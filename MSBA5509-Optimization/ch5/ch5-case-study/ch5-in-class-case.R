...
for (i in 1:num_simulations) {
  ...
  liquid <- runif(1, min = 2.5, max = 3.5)
  liquid2 <- runif(1, min = 1.5, max = 2.5)
...
best_solution <- results[[which.min(p_values)]]
# which.min() returns the index of the maximum value in the vector p_values
# It finds the position of the element with the highest value.

best_p <- min(p_values)
#finds the actual value
